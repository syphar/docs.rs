    ...
    pub fn new(meter: &metrics::Meter) -> Self {
        ...
        Self {
            invalidation_time: meter
                ...
                .build(),
            queue_time: meter.f64_histogram(format!("{PREFIX}.queue_time")).build(),
        }
    }
}
...
mod tests {
    ...
    fn config_with_cdn() -> crate::config::ConfigBuilder {
    ...
    fn otel_metrics(env: &TestEnvironment) -> CdnMetrics {
        CdnMetrics::new(&env.metric_provider().meter("test"))
    }
    ...
    async fn dont_create_invalidations_when_too_many_are_active() -> Result<()> {
        ...
        assert!(
            queued_or_active_crate_invalidations(&mut conn)
                .await?
                .iter()
                .filter(|i| i.cdn_distribution_id == DISTRIBUTION_ID_WEB
                    && !matches!(
                        &i.cdn_reference,

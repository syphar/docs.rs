...
fn capture_rustwide_step<T>(
    ...
    storage.set_max_size(max_log_size.as_usize());
...
impl<'build, 'ws> ReleaseBuild<'build, 'ws> {
    ...
    /// Build rustdoc JSON and HTML, with optional coverage, for one target.
    ///
    /// Coverage and lockfile retry are disabled by default. Enable them with
    /// `build_coverage(true)` and `retry_without_lockfile(true)`, respectively.
    /// [`Self::build_docs`] enables both for the default target only.
    ///
    ...
    pub fn build_target(
        ...
        if retry_without_lockfile
            && matches!(
                ...
                Err(StepReport {
                    value: BuildStepError::Prepare(_) | BuildStepError::Command(_),
                    ..
                })
            )
...
mod tests {
    ...
    fn retains_lockfile_regeneration_failure(build_release: bool) -> Result<()> {
        ...
        let failure = target
            ...
            .as_ref()
            .expect_err("regeneration failure retained");
...
mod policy_tests {
    ...
    fn auxiliary_failure_does_not_prevent_html(mode: &str) -> Result<()> {
        ...
        let release = environment
            .release(&fixture())
            .run(|build| {
                install_failing_rustdoc_wrapper(&build, mode)?;
                Ok(build.build_docs())
            })?
            .into_inner();
        assert!(release.has_docs());
    ...
    "#
                .parse()?;
                install_failing_rustdoc_wrapper(&build, "html")?;
                Ok(build.build_docs())
            })?
            .into_inner();
        assert!(!release.has_docs());
    ...
    fn html_command_retry_is_bounded(retry: bool, expected_attempts: usize) -> Result<()> {
        ...
        environment.release(&fixture()).run(|build| {
            ...
            let calls =
                fs::read_to_string(build.build.host_target_dir().join("rustdoc-calls.txt"))?;
            assert_eq!(
    ...
    #[test_case("docsrs-invalid-target", false, true, true; "preparation failure retries")]
    #[test_case("docsrs-invalid-target", false, false, true; "preparation retry disabled")]
    #[test_case("docsrs-invalid-target", false, true, false; "preparation failure without lockfile")]
    #[test_case(HOST_TARGET, true, true, true; "output failure does not retry")]
    #[ignore = "requires Docker and a Rust toolchain"]
    fn non_command_failure_retry_policy(
        target: &str,
        output_failure: bool,
        retry: bool,
        has_lockfile: bool,
        ...
        let mut environment = environment()?;
        environment.release(&fixture()).run(|build| {
            let lockfile = build.build.host_source_dir().join("Cargo.lock");
            ...
            let should_regenerate = !output_failure && retry && has_lockfile;
            assert_eq!(result.regenerate_lockfile().is_some(), should_regenerate);
            if should_regenerate {
                assert!(result.regenerate_lockfile().unwrap().is_ok());
            }
            Ok(())
        })?;
        Ok(())
